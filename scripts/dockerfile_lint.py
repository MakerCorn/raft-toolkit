...
class DockerfileLinter:
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if re.match(r'^FROM\s+', line, re.IGNORECASE):
                if ' as ' in line:  # lowercase 'as'
                    self.issues.append((i, "FromAsCasing", 
                                      "'as' should be uppercase 'AS' to match 'FROM'", 
                                      line.strip()))
                elif re.search(r'\s+as\s+', line):  # mixed case
                    self.issues.append((i, "FromAsCasing",
                                      "Use consistent uppercase: 'FROM ... AS ...'",
                                      line.strip()))
    ...
    def check_labels(self) -> None:
        ...
        if not has_maintainer:
            self.issues.append((0, "MissingLabel", 
                              "Consider adding LABEL maintainer", ""))
        if not has_description:
            self.issues.append((0, "MissingLabel", 
                              "Consider adding LABEL description", ""))
    ...
    def check_user_security(self) -> None:
        ...
        if not switches_to_user:
            self.issues.append((0, "SecurityIssue", 
                              "Consider using USER instruction for security", ""))
        elif not creates_user:
            self.issues.append((0, "SecurityWarning", 
                              "USER instruction found but no user creation detected", ""))
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if re.match(r'^RUN\s+', line, re.IGNORECASE):
                ...
                if 'apt-get install' in line and 'apt-get update' not in line:
                    ...
                    if not has_update:
                        self.issues.append((i, "AptGetUpdate", 
                                          "apt-get install should be preceded by apt-get update",
                                          line.strip()))
                ...
                if 'apt-get' in line and 'rm -rf /var/lib/apt/lists/*' not in line:
                    ...
                    if not any('rm -rf /var/lib/apt/lists/*' in l for l in 
                             self.lines[i-1:min(len(self.lines), i+5)]):
                        self.issues.append((i, "AptCacheCleanup", 
                                          "Consider cleaning apt cache after apt-get",
                                          line.strip()))
...
def main():
    ...
    if summary:
        ...
        critical_issues = sum(count for issue_type, count in summary.items() 
                            if issue_type in ['FromAsCasing', 'SecurityIssue'])
