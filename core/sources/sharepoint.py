...
class SharePointInputSource(BaseInputSource):
    ...
    def __init__(self, config):
        ...
        if not MSAL_AVAILABLE:
            raise SourceValidationError(
                "msal is required for SharePoint input sources. Install with: pip install msal"
            )
    ...
    async def _authenticate_client_credentials(self, creds: Dict[str, Any]) -> None:
        ...
        result = await asyncio.get_event_loop().run_in_executor(
            None, 
            lambda: app.acquire_token_for_client(scopes=scopes)
        )
    ...
    async def _authenticate_device_code(self, creds: Dict[str, Any]) -> None:
        ...
        flow = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: app.initiate_device_flow(scopes=scopes)
        )
    ...
    async def _get_site_info(self) -> None:
        ...
        response = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: self.session.get(graph_url)
        )
    ...
    async def _get_library_info(self) -> None:
        ...
        response = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: self.session.get(graph_url, params=params)
        )
    ...
    async def _get_library_items(self, folder_path: str = '') -> List[Dict[str, Any]]:
        ...
        while next_url:
            if next_url == graph_url:
                response = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.session.get(next_url, params=params)
                )
            else:
                response = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.session.get(next_url)
                )
    ...
    async def get_document(self, document: SourceDocument) -> SourceDocument:
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.session.get(download_url)
            )
