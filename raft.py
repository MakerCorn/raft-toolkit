...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--doctype", type=str, default="pdf", help="The type of the document, must be one of the accepted doctypes", choices=["pdf", "txt", "json", "api", "pptx"])
    parser.add_argument("--openai_key", type=str, default=None, help="Your OpenAI key used to make queries to GPT-3.5 or GPT-4")
    parser.add_argument("--embedding_model", type=str, default="nomic-embed-text", help="The embedding model to use to encode documents chunks (text-embedding-ada-002, ...)")
    parser.add_argument("--completion_model", type=str, default="llama3.2", help="The model to use to generate questions and answers (gpt-3.5, gpt-4, ...)")
    parser.add_argument("--system-prompt-key", default="gpt", help="The system prompt to use to generate the dataset")
    parser.add_argument("--embed-workers", type=int, default=1, help="The number of worker threads to use to generate embeddings")
    ...
    parser.add_argument("--use-azure-identity", type=bool, default=False, help="Use MS Entra ID for token retrieval")
    parser.add_argument("--pace", type=bool, default=True, help="Whether to pace the calls to the LLM to stay below the Token/Minute limits")
...
def get_doc_chunks(
    ...
    if doctype == "json":
        ...
        raise TypeError("Document is not one of the accepted types: api, pdf, json, txt, pptx")
...
def build_or_load_chunks(
    ...
    if not chunks:
        chunks = get_chunks(datapath, doctype, CHUNK_SIZE, OPENAPI_API_KEY, model=embedding_model, use_azure_identity=use_azure_identity, embed_workers=embed_workers, pace=pace)
...
def main():
    ...
    prompt_templates[args.system_prompt_key+"_qa"] = load_prompt_template(args.templates + args.system_prompt_key + '_qa_template.txt')
    ...
    # Chunks
    chunks = build_or_load_chunks(datapath, args.doctype, CHUNK_SIZE, OPENAPI_API_KEY, args.embedding_model, checkpoints_dir, args.use_azure_identity, args.embed_workers, args.pace)
    ...
    cot_answers_ds = stage_generate(chat_completer, checkpoints_dir, chunks, num_questions, max_workers, doctype, completion_model, system_prompt_key, num_distract=NUM_DISTRACT_DOCS, use_azure_identity=args.use_azure_identity, p=args.p)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        logger.error(f"Error processing chunk: {e} ID: {i} Data: {chunks[i]}")
        else:            
            for i in missing_chunks:
                try:
