...
try:
    from core.logging.setup import log_setup, get_logger, configure_logging, setup_sentry_logging
except ImportError:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-include-patterns", type=str,
                        help="JSON array of glob patterns to include")
    ...
    parser.add_argument("--source-batch-size", type=int, default=100,
                        help="Batch size for processing source files")
    ...
    parser.add_argument("--rate-limit-max-retries", type=int, default=3,
                        help="Maximum number of retries on rate limit errors")
...
def main():
    ...
    try:
        ...
        if hasattr(logger, 'start_operation'):
            logger.start_operation("raft_dataset_generation", 
                                 input_path=input_path,
                                 source_type=config.source_type,
                                 doctype=config.doctype,
                                 chunk_strategy=config.chunking_strategy,
