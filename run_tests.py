    ...
    if args.coverage:
        ...
        if args.output_dir:
            ...
            coverage_args.extend(
                [f"--cov-report=html:{coverage_path}/htmlcov", f"--cov-report=xml:{Path(args.output_dir)}/coverage.xml"]
            )
        else:
            # Put both in coverage dir
            coverage_args.extend(
                [f"--cov-report=html:{coverage_path}/htmlcov", f"--cov-report=xml:{coverage_path}/coverage.xml"]
            )
